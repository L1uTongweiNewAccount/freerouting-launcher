...
var freerouting_jar = path.join(os.homedir(), ".freerouting", "freerouting.jar");
...
.default('executable', freerouting_jar).option('executable', {type: "string", describe: "Path to freerouting executable jar File.", alias: 'jar'})
...
if(argv.executable == freerouting_jar && !fs.existsSync(freerouting_jar)){
    console.log("Downloading freerouting-1.9.0.jar to " + freerouting_jar + "...");
    fs.mkdirSync(path.join(os.homedir(), ".freerouting"));
    getFile("https://github.com/freerouting/freerouting/releases/download/v1.9.0/freerouting-1.9.0.jar", freerouting_jar).then(() => {
        console.log("Download completed. Please re-run me.");
        process.exit(1);
    });
    while(true);
}
